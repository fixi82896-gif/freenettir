...
def save_history(history):
    ...
    history["leftover_configs"] = history["leftover_configs"][-200:]
    history["leftover_proxies"] = history["leftover_proxies"][-200:]
...
def collect_configs(history, sent_hashes_set):
    ...
    primary_configs = []
    try:
        res = HTTP_SESSION.get("https://manager.onetwothree123.ir/", timeout=10)
        if res.status_code == 200:
    ...
    if len(valid_configs) < 100:
        ...
        for src in github_sources:
            try:
                r = HTTP_SESSION.get(src, timeout=8)
...
def collect_proxies(history, sent_proxies_set):
    ...
    primary_proxies = []
    try:
        res = HTTP_SESSION.get("https://office.onetwothree123.ir/", timeout=10)
        if res.status_code == 200:
...
def main():
    ...
    valid_configs = collect_configs(history, sent_hashes_set)
    ...
    if len(valid_configs) < 3:
        print("⚠️ تعداد کانفیگ‌های موجود کمتر از ۳ عدد است. این اجرا لغو می‌شود.")
    ...
    # ---------------------------------------------------------------------
    # 📝 پایان ۶۰ دقیقه ارسال پست‌ها و ارسال فایل متنی
    # ---------------------------------------------------------------------
    ...
    try:
        ...
        if os.path.exists(config_file_name):
            ...
            print(f"🗑️ فایل موقت {config_file_name} حذف شد.")

    # 💾 ذخیره آنلاین تاریخچه در گیت‌هاب
